...
CREATE PROCEDURE public.proc_update_absensi(IN p_id_mhs integer, IN p_action character varying, IN p_keterangan text, OUT p_id_absensi integer, OUT p_result_code integer, OUT p_result_message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.mahasiswa WHERE id_mahasiswa = p_id_mhs) THEN
        p_result_code := -1;
    ...
    FROM public.absensi
    WHERE id_mhs = p_id_mhs AND tanggal = v_tanggal_hari_ini;
    ...
    IF p_action = 'checkin' THEN
        ...
        ELSE
            INSERT INTO public.absensi (
                id_mhs, tanggal, waktu_datang, keterangan
            )
            VALUES (
                p_id_mhs, v_tanggal_hari_ini, CURRENT_TIMESTAMP, p_keterangan
            )
...
ALTER PROCEDURE public.proc_update_absensi(IN p_id_mhs integer, IN p_action character varying, IN p_keterangan text, OUT p_id_absensi integer, OUT p_result_code integer, OUT p_result_message character varying) OWNER TO postgres;
