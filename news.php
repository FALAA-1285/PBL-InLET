...
if (!empty($search_query) && is_numeric($search_query)) {
    $stmt = $conn->prepare("SELECT COUNT(*) FROM berita WHERE judul = :search");
    $stmt->execute([':search' => (int)$search_query]);
    $total_items_news = $stmt->fetchColumn();
    
    // Get news with pagination and search
    $stmt = $conn->prepare("SELECT * FROM berita WHERE judul = :search ORDER BY judul LIMIT :limit OFFSET :offset");
    $stmt->bindValue(':search', (int)$search_query, PDO::PARAM_INT);
} else {
    $stmt = $conn->query("SELECT COUNT(*) FROM berita");
    $total_items_news = $stmt->fetchColumn();
    
    // Get articles with pagination
    $stmt = $conn->prepare("SELECT * FROM berita ORDER BY judul LIMIT :limit OFFSET :offset");
}
...
if (empty($all_gallery)) {
    for ($i = 1; $i <= 80; $i++) {
        $w = rand(320, 460);
        $h = rand(240, 500);
        $all_gallery[] = ["img" => "https://picsum.photos/seed/news{$i}/{$w}/{$h}", "judul" => "Gallery Image {$i}"];
    }
} else {
            ...
            <div class="container text-center">
                ...
                <div class="row g-4 justify-content-center">
                    <?php if (empty($news_list)): ?>
                        <div class="col-12">
                            <p style="color: var(--gray); padding: 3rem;">
                                <?php if (!empty($search_query)): ?>
                    ...
                    <nav aria-label="News pagination" style="margin-top: 3rem;">
                        <ul class="pagination justify-content-center" style="gap: 0.5rem;">
                            ...
                            <?php if ($current_page_news > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= $page_url ?>page_news=<?php echo $current_page_news - 1; ?>#focus-areas" aria-label="Previous">
                            ...
                            <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                                <li class="page-item <?php echo ($i == $current_page_news) ? 'active' : ''; ?>">
                                    <a class="page-link" href="<?= $page_url ?>page_news=<?php echo $i; ?>#focus-areas"><?php echo $i; ?></a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <?php if ($end_page < $total_pages_news): ?>
                                ...
                                <?php endif; ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= $page_url ?>page_news=<?php echo $total_pages_news; ?>#focus-areas"><?php echo $total_pages_news; ?></a>
                                </li>
                            <?php endif; ?>
                            
                            <?php if ($current_page_news < $total_pages_news): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= $page_url ?>page_news=<?php echo $current_page_news + 1; ?>#focus-areas" aria-label="Next">
